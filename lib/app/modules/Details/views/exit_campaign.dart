...
class ExitCampaignScreen extends StatelessWidget {
  final controller = Get.put(ExitCampaignController());
  final themeController = Get.put(ThemeController());
  final GlobalKey<FormState> exitCampaignFormGlobalKey = GlobalKey<FormState>();

  ExitCampaignScreen({super.key});

  @override
  Widget build(BuildContext context) {
    return GetBuilder<ExitCampaignController>(
        init: ExitCampaignController(),
        builder: (context) {
          return Scaffold(
            appBar: CustomAppBar(
              appBarTitleText: strExitCampaign.toUpperCase(),
            ...
            body: Column(
              children: [
                Expanded(
                    child: Form(
                  key: exitCampaignFormGlobalKey,
                  child: ListView(
                    children: [
                      _requiredTitle(title: strReasonForExit),
                ...
                BottomButtonWidget(
                  onPressed: () {
                    if (exitCampaignFormGlobalKey.currentState!.validate()) {
                      controller.hitExitCampaignApi();
                    }
                  },
                  btnTitle: strSubmit,
                  isBorderColor: false,
